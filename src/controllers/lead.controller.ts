...
export class LeadController {
  ...
  async getById(req: Request, res: Response) {
    try {
      ...
      return res.status(400).json({ success: false, message: error?.message || "Failed to fetch lead" });
    }
  }
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      return res.status(400).json({ success: false, message: error?.message || "Failed to update lead" });
    }
  }
